...
var RobotArm = function (canvas) {
    ...
    self.examLevel = function (level) {
        ...
        var amount = level == 1? 7 : (Math.floor(Math.random() * Math.floor(6))+2);

        for (var b=0; b<=amount; b++){
            map[0][b] = local.blocks.availableColors[Math.floor(Math.random() * local.blocks.availableColors.length)];
        }
